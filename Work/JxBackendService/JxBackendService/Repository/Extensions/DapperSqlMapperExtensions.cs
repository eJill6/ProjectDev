...
namespace JxBackendService.Repository.Extensions
{
    ...
    {
        private static readonly int _stringDefaultSize = 10;
        ...
        {
            Type type = typeof(T);
            List<PropertyInfo> keyList = ModelUtil.GetKeys<T>();
        ...
        public static long SaveByProcedure<T>(this IDbConnection connection, ActTypes actType, string tableName, T model) where T : class
        {
            ...
            {
                columnNameList = query.ToList();
                columnNameList.RemoveAll(r => r.Equals(nameof(BaseEntityModel.CreateDate), StringComparison.CurrentCultureIgnoreCase) ||
