...
namespace GenerateStoredProcedures
{
    ...
    class Program
    {
        ...
        {
            string template = $"{seq.ToString().PadLeft(2, '0')}-{action}-{GetDbName()}-{dbObjectName}" + "{0}" + ".sql";
        ...
        private static string GetDeclareParameters(ActType actType, HashSet<string> keys, List<ColumnInfo> columninfos)
        {
            ...
            for (int i = 0; i < requiredColumns.Count; i++)
            {
                ...
                var requiredColumn = requiredColumns[i];
                ...
                if (requiredColumn.DataType.Equals("decimal", StringComparison.CurrentCultureIgnoreCase)
                    && requiredColumn.Precision.HasValue && requiredColumn.Scale.HasValue)
                {
...
GRANT EXECUTE ON [dbo].[{procedureName}] TO [polyqqccwin];
GRANT EXECUTE ON [dbo].[{procedureName}] TO [polyqqccwinpouch];
GO
";
