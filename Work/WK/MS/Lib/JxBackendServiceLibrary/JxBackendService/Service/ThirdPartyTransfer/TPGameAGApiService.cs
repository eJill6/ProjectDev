...
namespace JxBackendService.Service.ThirdPartyTransfer
{
    public abstract class TPGameAGApiService : BaseTPGameApiService
    {
        ...
        protected override DetailRequestAndResponse GetRemoteOrderApiResult(string tpGameAccount, BaseTPGameMoneyInfo tpGameMoneyInfo)
        {
            ...
            _debugUserService.ForcedDebug(tpGameMoneyInfo.UserID, new { queryString, client.Url }.ToJsonString());
        ...
        protected override DetailRequestAndResponse GetRemoteTransferApiResult(bool isMoneyIn, string tpGameAccount, BaseTPGameMoneyInfo tpGameMoneyInfo)
        {
            ...
            BaseReturnDataModel<DetailRequestAndResponse> prepareResult = PrepareTransferCredit(isMoneyIn, tpGameAccount, tpGameMoneyInfo);

            if (!prepareResult.IsSuccess)
            {
                LogUtil.Error(prepareResult.Message);
            ...
            return GetTransferCreditConfirmApiResult(isMoneyIn, tpGameAccount, tpGameMoneyInfo);
        }

        protected override string GetRemoteUserScoreApiResult(string tpGameAccount)
        {
            return BaseGetRemoteUserScoreApiResult(tpGameAccount).DataModel;
        }
        ...
        protected override BaseReturnModel CheckOrCreateRemoteAccount(CreateRemoteAccountParam param)
        {
            ...
            {
                LogUtil.Error("用户" + param.TPGameAccount + "在AG平台创建账号失败，错误信息：" + errorMsg);
        ...
        {
            return GetRemoteLoginApiResult(tpGameAccount, ip, isMobile, loginInfo);
        }
        ...
        protected override BaseReturnDataModel<string> GetRemoteCreateAccountApiResult(CreateRemoteAccountParam param)
        {
            ...
            _debugUserService.ForcedDebug(EnvLoginUser.LoginUser.UserId, new { queryString, client.Url }.ToJsonString());
        ...
        {
            string gameType = GetThirdPartyRemoteCode(loginInfo);
            ...
            string param = $@"cagent={_agAppSetting.VendorID}/\\\\/loginname={tpGameAccount}/\\\\/actype={_agAppSetting.AcType}/\\\\/" +
                $@"password={password}/\\\\/dm={_agAppSetting.DM}/\\\\/sid={sid}/\\\\/lang=1/\\\\/gameType={gameType}" +
        ...
        private BaseReturnDataModel<string> BaseGetRemoteUserScoreApiResult(string tpGameAccount)
        {
            HttpClient client = CreateAGHttpClient();

            //get userid, username from tpGameAccount
            Dictionary<string, BaseBasicUserInfo> userMap = _tpGameAccountReadService.GetUsersByTPGameAccounts(
                PlatformProduct.AG,
                new HashSet<string>() { tpGameAccount });

            BaseBasicUserInfo user = userMap[tpGameAccount];
            string password = CreateAGGamePwd(user.UserId);
            ...
            _debugUserService.ForcedDebug(user.UserId, new { param, client.Url }.ToJsonString());
            string apiResult = client.GetString();
        ...
        /// 预备转账
        /// </summary>
        protected virtual BaseReturnDataModel<DetailRequestAndResponse> PrepareTransferCredit(bool isMoneyIn, string tpGameAccount, BaseTPGameMoneyInfo tpGameMoneyInfo)
        {
            /*
            /// 0 成功
            ...
            string queryString = string.Format("cagent={0}/\\\\/method=tc/\\\\/loginname={1}/\\\\/billno={2}/\\\\/type={3}" +
                ...
                _agAppSetting.AcType,
                CreateAGGamePwd(tpGameMoneyInfo.UserID),
                _agAppSetting.Currency);
        ...
        /// 确认转账Api Result
        /// </summary>
        protected virtual DetailRequestAndResponse GetTransferCreditConfirmApiResult(bool isMoneyIn, string tpGameAccount, BaseTPGameMoneyInfo tpGameMoneyInfo)
        {
            HttpClient client = CreateAGHttpClient();
        ...
        private class HttpClient
        {
            ...
            public string GetString()
            {
                ...
                else
                {
                    ...
                    {
                        LogUtil.ForcedDebug($"HttpClient.GetString() ArgumentException = {ex.ToString()}");
                        encoding = defaultEncoding;
                    }
                }
