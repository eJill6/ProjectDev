...
namespace JxBackendService.Service.ThirdPartyTransfer
{
    public abstract class TPGameWLBGApiService : BaseTPGameApiService, ITPGameWLBGApiService
    {
        ...
        {
            var request = new WLBGGetBalanceRequestModel
            {
                Uid = tpGameAccount
            };

            var returnModel = DoGetRequest(WLBGSharedAppSetting.GetBalance, request, out DetailRequestAndResponse detail);
        ...
        public override BaseReturnDataModel<UserScore> GetTransferReturnModel(string apiResult)
        {
            ...
            return new BaseReturnDataModel<UserScore>(transferModel.Msg ?? transferModel.Data.Reason);
        }
        ...
        public override BaseReturnModel GetQueryOrderReturnModel(string apiResult)
        {
            ...
            else if (responseModel.Data.Status == WLBGResponseStatus.Failed)
            {
                return new BaseReturnDataModel<UserScore>(responseModel.Data.Reason);
            }
            else if (responseModel.Data.Status == WLBGResponseStatus.NotFound)
            {
                return new BaseReturnDataModel<UserScore>(responseModel.Data.Reason);
            }
        ...
        {
            string subGameCode = GetThirdPartyRemoteCode(loginInfo);
