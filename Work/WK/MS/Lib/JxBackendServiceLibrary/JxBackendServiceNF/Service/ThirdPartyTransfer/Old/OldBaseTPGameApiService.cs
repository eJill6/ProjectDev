...
namespace JxBackendService.Service.ThirdPartyTransfer.Old
{
    public abstract class OldBaseTPGameApiService<ApiParamType> : BaseService
    {
        ...
        public void TransferHandle(object obj)
        {
            try
            {
                ...
                if (returnModel.IsSuccess)
                {
                    ...
                    LogUtilService.ForcedDebug($"遠端{transferActionName}成功等待確認. " +
                        $"Request:{requestJson} " +
        ...
        public void CheckProcessingOrderAndSaveToRemoteDB(object state)
        {
            ...
            {
                LogUtilService.Error(new NotSupportedException("param is not OldTPGameOrderParam<ApiParamType> "));
                return;
            }
        ...
        private bool RecheckAfterTransfer(OldTPGameOrderParam<ApiParamType> oldTPGameOrderParam, UserScore remoteUserScore)
        {
            ...
            {
                //其餘異常狀況,不做任何處理
                LogUtilService.Error($"资料回传异常，无法取得数据或解析, info={oldTPGameOrderParam.TPGameMoneyInfo.ToJsonString()}");
                return false;
            }

            if (remoteOrderResult.IsSuccess)
            {
                LogUtilService.ForcedDebug($"远程确认{transferActionName}成功，info={oldTPGameOrderParam.TPGameMoneyInfo.ToJsonString()}");
                ...
                if (remoteUserScore != null)
                {
                    ...
                    if (saveResult.IsSuccess)
                    {
                        ...
                        LogUtilService.ForcedDebug($"本地{transferActionName}成功，MoneyID：{oldTPGameOrderParam.TPGameMoneyInfo.GetMoneyID()}");

                        return true;
                    }
                    else
                    {
                        LogUtilService.ForcedDebug($"本地{transferActionName}失败，info={new { saveResult.Message, oldTPGameOrderParam.TPGameMoneyInfo }.ToJsonString()}");
            ...
            else
            {
                ...
                {
                    LogUtilService.Error(new { oldTPGameOrderParam.TPGameMoneyInfo, saveOrderFailResult.Message }.ToJsonString());
