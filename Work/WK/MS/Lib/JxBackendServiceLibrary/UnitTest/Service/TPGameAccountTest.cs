...
namespace UnitTest.Util
{
    ...
    public class TPGameAccountTest : BaseTest
    {
        ...
        {
            BaseReturnDataModel<UserAccountSearchResult> returnModel = _tpGameAccountReadService.Value.GetByTPGameAccount(null, "mslD_888");
            Assert.AreEqual(ReturnCode.Success.Value, returnModel.Code);

            BaseReturnDataModel<UserAccountSearchResult> returnModelByUserId = _tpGameAccountReadService.Value.GetByLocalAccount(588);
            Assert.AreEqual(ReturnCode.Success.Value, returnModel.Code);
        }
