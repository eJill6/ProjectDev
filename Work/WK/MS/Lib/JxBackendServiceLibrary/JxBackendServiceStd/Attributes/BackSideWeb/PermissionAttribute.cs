...
namespace JxBackendService.Attributes.BackSideWeb
{
    public class PermissionAttribute : Attribute, IAuthorizationFilter
    {
        ...
        public void OnAuthorization(AuthorizationFilterContext context)
        {
            ...
            var backSideWebLoginUserService = DependencyUtil.ResolveService<IBackSideWebUserService>().Value;
            BackSideWebUser backSideWebUser = backSideWebLoginUserService.GetUser();

            if (backSideWebUser == null)
            {
                context.Result = _noPermissionActionService.Value.GetRedirectToLoginPage();
            ...
            if (!hasPermission)
            {
                ...
                if (httpContextService.Value.IsAjaxRequest())
                {
                    context.Result = _noPermissionActionService.Value.GetNoPermissionJsonResult();

                    return;
                }
                else
                {
                    context.Result = _noPermissionActionService.Value.GetRedirectToNoPermissionPage();
