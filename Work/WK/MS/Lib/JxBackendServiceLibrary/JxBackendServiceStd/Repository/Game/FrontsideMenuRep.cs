...
namespace JxBackendService.Repository.Game
{
    public class FrontsideMenuRep : BaseDbRepository<FrontsideMenu>, IFrontsideMenuRep
    {
        ...
        public List<FrontsideMenu> GetGameCenterMenusByProduct(string productCode)
        {
            string sql = GetAllQuerySQL(InlodbType.Inlodb) + " WHERE ProductCode = @productCode AND ISNULL(RemoteCode, '') = '' ORDER BY Sort";
         
            return DbHelper.QueryList<FrontsideMenu>(sql, new { productCode = productCode.ToNVarchar(50) });                     
        }

        public bool IsFrontsideMainMenuExists(FrontSideMainMenu frontsideMainMenu)
        {
            ...
            List<FrontsideMenu> frontsideMenus = DbHelper.QueryList<FrontsideMenu>(sql, frontsideMainMenu);
            frontsideMenus.RemoveAll(r => r.Type == FrontsideMenuTypeSetting.Hot || !r.RemoteCode.IsNullOrEmpty());

            return frontsideMenus.Any();
        }
