...
namespace JxBackendService.Repository.Base
{
    public abstract class BaseDbRepository : BaseDbRepository<object>
    {
        ...
        protected override BaseReturnDataModel<long> BaseCreate(InlodbType inlodbType, object model, string tableName)
        {
    ...
    public abstract class BaseDbRepository<T> : IBaseDbRepository<T> where T : class
    {
        ...
        public BaseReturnDataModel<long> CreateByProcedure(InlodbType inlodbType, T model)
        {
            return BaseCreate(inlodbType, model, null);
        }

        public BaseReturnDataModel<long> CreateByProcedure(T model)
        {
            return BaseCreate(inlodbType: null, model, tableName: null);
        }

        public BaseReturnDataModel<long> CreateByProcedure(T model, string tableName)
        {
            return BaseCreate(inlodbType: null, model, tableName);
        }
        ...
        protected virtual BaseReturnDataModel<long> BaseCreate(InlodbType inlodbType, T model, string tableName)
        {
            ...
            returnDataModel.DataModel = conn.SaveByProcedure(inlodbType, ActTypes.Insert, tableName, model);
        ...
        public bool UpdateByProcedure(InlodbType inlodbType, T model)
        {
            return BaseUpdate(inlodbType, model, isSystemJob: false);
        }

        public bool UpdateByProcedure(T model)
        {
            return BaseUpdate(inlodbType: null, model, isSystemJob: false);
        }

        public bool UpdateByProcedure(T model, bool isSystemJob)
        {
            return BaseUpdate(inlodbType: null, model, isSystemJob);
        }
        ...
        private bool BaseUpdate(InlodbType inlodbType, T model, bool isSystemJob)
        {
            ...
            IDbConnection conn = GetDbConnection();
            bool isSuccess = conn.SaveByProcedure(inlodbType, ActTypes.Update, model) > 0;
            //if (isSaveByProcedure)
            //{
        ...
        public bool DeleteByProcedure(InlodbType inlodbType, T model)
        {
            return BaseDelete(inlodbType, model);
        }

        public bool DeleteByProcedure(T model)
        {
            return BaseDelete(inlodbType: null, model);
        }

        private bool BaseDelete(InlodbType inlodbType, T model)
        {
            //T beforeSaveModel = GetSingleByKey(model, DbConnTypes.ReadWrite);

            IDbConnection conn = GetDbConnection();
            bool isSuccess = conn.SaveByProcedure(inlodbType, ActTypes.Delete, model) > 0;
            //if (isSaveByProcedure)
            //{
        ...
        {
            IAppSettingService appSettingService = DependencyUtil.ResolveKeyed<IAppSettingService>(jxApplication, SharedAppSettings.PlatformMerchant).Value;
