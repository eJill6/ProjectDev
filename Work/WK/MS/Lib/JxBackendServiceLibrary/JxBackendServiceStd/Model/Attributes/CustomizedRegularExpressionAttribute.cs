...
namespace JxBackendService.Model.Attributes
{
    ...
    public class RegularExpressionType : BaseValueModel<RegularExpressionEnumTypes, RegularExpressionType>
    {
        ...
        public static readonly RegularExpressionType OneIntAndTwoDigits = new RegularExpressionType()
        {
            Value = RegularExpressionEnumTypes.OneIntAndTwoDigits,
            ErrorMsgResourceType = typeof(MessageElement),
            ErrorMsgResourcePropertyName = nameof(MessageElement.FieldFormatFail),
            Pattern = @"^\d{1}(\.\d{1,4})?$"
        };
    }
