...
namespace JxBackendService.Service
{
    ...
    {
        private readonly Lazy<ITransferCompensationRep> _transferCompensationRep;
        ...
        {
            UserInfoAdditional userInfoAdditional = _userInfoAdditionalService.Value.GetSingle(param.UserID);
            ...
            bool hasCompensation = _transferCompensationRep.Value.HasUnProcessedCompensation(
            ...
            return _transferCompensationRep.Value.CreateByProcedure(createCompensation).CastByJson<BaseReturnModel>();
        }

        public BaseReturnModel ProcessedMoneyOutCompensation(ProcessedCompensationParam param)
        {
            List<TransferCompensation> moneyOutCompensations = _transferCompensationRep.Value.GetUserUnProcessedCompensations(
                new SearchUserCompensationParam
                {
                    UserID = param.UserID,
                    ProductCode = param.ProductCode,
            ...
            foreach (TransferCompensation compensation in moneyOutCompensations)
            {
                ...
                _transferCompensationRep.Value.UpdateByProcedure(compensation);
            }
        ...
        public List<int> GetTransferCompensationUserIds(PlatformProduct product)
        {
            ...
            // 先取得目前未處理的補償單
            List<TransferCompensation> moneyOutCompensations = _transferCompensationRep.Value.GetUnProcessedCompensations(
            ...
            List<TPGameMoneyOutInfo> processedMoneyOut = tpGameStoredProcedureRep.Value.GetTPGameProcessedMoneyOutInfo(minCreateDate, DateTime.Now, userIds);
            ...
            foreach (int userId in userIds)
            {
                ...
                var transferResultMap = _jxCacheService.Value.GetCache<Dictionary<string, BaseReturnDataModel<decimal>>>(transferOutAllKey);
