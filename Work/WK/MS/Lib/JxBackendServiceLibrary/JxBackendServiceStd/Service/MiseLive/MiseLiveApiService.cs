...
{
    public class MiseLiveApiService : BaseEnvLoginUserService, IMiseLiveApiService
    {
        #region api path
        ...
        protected virtual int MiseLiveRequestWaitMilliSeconds => 5 * 1000;

        private readonly Lazy<IHttpWebRequestUtilService> _httpWebRequestUtilService;

        private readonly Lazy<IMiseLiveAppSettingService> _miseLiveAppSettingService;

        private readonly Lazy<ILogUtilService> _logUtilService;

        public MiseLiveApiService(EnvironmentUser envLoginUser) : base(envLoginUser)
        {
        ...
        protected virtual string GetUserBalanceApiRemoteResult(IMiseLiveUserBalanceRequestParam requestParam)
        {
            ...
            request.Salt = _miseLiveAppSettingService.Value.MSSealSalt;

            string requestUrl = _httpWebRequestUtilService.Value.CombineUrl(
                _miseLiveAppSettingService.Value.MSSealAddress,
                s_balancePath,
        ...
        protected virtual string GetTransferResultApiRemoteResult(IMiseLiveTransferOrderRequestParam requestParam)
        {
            ...
            Dictionary<string, object> routeValues = RouteUtil.ConvertToRouteValues(
                request,
                (property) =>
                {
        ...
        private string DoRequestWithLog<T>(string purpose, HttpMethod httpMethod, string requestUrl, T request) where T : IMiseLiveRequest
        {
            ...
            try
            {
                var webRequestParam = new WebRequestParam()
                {
                    ...
                    Headers = headers,
                    Purpose = purpose,
                    TimeOut = MiseLiveRequestWaitMilliSeconds
                };

                responseString = HttpWebRequestUtil.GetResponse(webRequestParam, out HttpStatusCode httpStatusCode);
            }
            finally
            {
                _logUtilService.Value.ForcedDebug($"url={requestUrl}, request={request.ToJsonString(ignoreNull: true, isCamelCaseNaming: true)}, " +
                    $"header={headers.ToJsonString()}, response={responseString}");
            }
    ...
    [MockService]
    public class MiseLiveApiMockService : MiseLiveApiService
    {
        public MiseLiveApiMockService(EnvironmentUser envLoginUser) : base(envLoginUser)
        {
        }

        protected override int MiseLiveRequestWaitMilliSeconds => 100 * 1000;

        //protected override string GetTransferInApiRemoteResult(IMiseLiveTransferRequestParam requestParam)
        //{
        //    var response = new MiseLiveResponse<MiseLiveBalance>()
        //    {
        //        Success = false,
        //        Error = MiseLiveMessage.InvalidUser.Value
        //    };

        //    return response.ToJsonString(isCamelCaseNaming: true);
        //}

        //protected override string GetTransferOutApiRemoteResult(IMiseLiveTransferRequestParam requestParam)
        //{
        //    var response = new MiseLiveResponse<MiseLiveBalance>()
        //    {
        //        Success = false,
