...
namespace JxBackendService.Service.MiseLive
{
    ...
    {
        private readonly Lazy<ITPGameOBEBApiService> _tpGameOBEBApiService;
        ...
        {
            OBEBBaseResponseWtihDataModel<List<OBEBAnchor>> anchorsResult = _tpGameOBEBApiService.Value.GetAnchorsResult();

            if (!anchorsResult.IsSuccess ||
                anchorsResult == null ||
                anchorsResult.data == null)
            {
                return null;
            }

            //轉換為平台自己定義的model, 這裡是第一家串, 所以欄位是一樣的, 可以直接cast過去
            return anchorsResult.data
                .Select(s => s.CastByJson<MiseLiveAnchor>() as IMiseLiveAnchor)
                .Where(a => a.LiveStatus == 1)
                .ToList();
        }
    }
}