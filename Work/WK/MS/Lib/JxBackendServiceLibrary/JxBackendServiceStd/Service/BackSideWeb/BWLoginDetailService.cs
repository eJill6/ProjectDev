...
namespace JxBackendService.Service.BackSideWeb
{
    ...
    {
        private readonly Lazy<IBWLoginDetailRep> _bwLoginDetailRep;
        ...
        {
            string clientIP = _ipUtilService.Value.GetIPAddress();

            return _bwLoginDetailRep.Value.CreateByProcedure(new BWLoginDetail()
            {
                UserID = param.UserID,
