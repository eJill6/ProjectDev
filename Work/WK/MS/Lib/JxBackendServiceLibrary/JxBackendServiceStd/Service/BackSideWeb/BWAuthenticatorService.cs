...
namespace JxBackendService.Service.BackSideWeb
{
    public class BWAuthenticatorService : BaseBackSideService, IBWAuthenticatorService
    {
        ...
        protected readonly Lazy<IAppSettingService> _appSettingService;

        protected readonly Lazy<IMerchantSettingService> _merchantSettingService;

        private readonly Lazy<IBWUserAuthenticatorRep> _bwUserAuthenticatorRep;
        ...
        {
            _appSettingService = ResolveKeyed<IAppSettingService>(envLoginUser.Application);
        ...
        public BaseReturnModel IsVerifyCodeValid(ValidVerifyCodeParam validVerifyCodeParam)
        {
            ...
            string accountSecretKey = userAuthenticator.EncryptAccountSecretKey.ToDescryptedData(_appSettingService.Value.CommonDataHash);
        ...
        public BaseReturnDataModel<QrCodeViewModel> GetQrCode(CreateQrCodeViewModelParam createQrCodeViewModelParam)
        {
            ...
            BWUserInfo userInfo = _bwUserInfoRep.Value.GetSingleByKey(InlodbType.Inlodb, new BWUserInfo { UserID = createQrCodeViewModelParam.UserId });
            ...
            if (!createQrCodeViewModelParam.IsForcedRefresh)
            {
                ...
                {
                    accountSecretKey = userAuthenticator.EncryptAccountSecretKey.ToDescryptedData(_appSettingService.Value.CommonDataHash);
            ...
            if (createQrCodeViewModelParam.IsForcedRefresh)
            {
                accountSecretKey = CreateNewUserSecretKey(
                    ...
                    (secretKey) =>
                    {
                        return secretKey.ToEncryptedData(_appSettingService.Value.CommonDataHash);
                    });
        ...
        {
            return _bwUserAuthenticatorRep.Value.GetSingleByKey(InlodbType.Inlodb, new BWUserAuthenticator() { UserID = userId }, false);
        }
            ...
            Func<string, string> getEncryptKey)
        {
            ...
            BWUserAuthenticator userAuthenticator = _bwUserAuthenticatorRep.Value.GetSingleByKey(InlodbType.Inlodb,
            ...
            if (userAuthenticator != null)
            {
                ...
                _bwUserAuthenticatorRep.Value.UpdateByProcedure(userAuthenticator);
            }
            else
            {
                ...
                _bwUserAuthenticatorRep.Value.CreateByProcedure(userAuthenticator);
            }
        ...
        private SetupCode GetSetupCode(CreateQrCodeImageParam createQrCodeImageParam)
        {
            ...
            string issuer = _merchantSettingService.Value.GoogleAuthenticatorIssuer;
