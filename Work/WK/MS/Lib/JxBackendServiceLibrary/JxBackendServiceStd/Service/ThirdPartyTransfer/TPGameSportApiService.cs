...
namespace JxBackendService.Service.ThirdPartyTransfer
{
    public abstract class TPGameSportApiService : BaseTPGameApiService
    {
        ...
        public override BaseReturnModel GetQueryOrderReturnModel(string apiResult)
        {
            ...
            LogUtilService.ForcedDebug($"远程查询订单状态失败 apiResult = {apiResult}");
        ...
        public override DetailRequestAndResponse GetRemoteTransferApiResult(bool isMoneyIn, CreateRemoteAccountParam createRemoteAccountParam, BaseTPGameMoneyInfo tpGameMoneyInfo)
        {
            ...
            string param = $"vendor_member_id={createRemoteAccountParam.TPGameAccount}&vendor_trans_id={tpGameMoneyInfo.OrderID}" +
        ...
        public override BaseReturnDataModel<UserScore> GetUserScoreReturnModel(string apiResult)
        {
            ...
            {
                LogUtilService.ForcedDebug($"GetUserScoreReturnModel balanceResponse is null, apiResult={apiResult}");

                return new BaseReturnDataModel<UserScore>("balanceResponse is null");
            }

            string responseErrorMessage = null;
            string dataErrorMessage = null;

            if (!balanceResponse.IsSuccess)
            {
                responseErrorMessage = balanceResponse.ErrorLog;
            }
            ...
            if (balanceResponse.Data != null)
            {
                ...
                {
                    dataErrorMessage = failBalanceData.ErrorLog;
                }
            }

            //兩個都有錯誤訊息的時候以dataErrorMessage為主
            if (!responseErrorMessage.IsNullOrEmpty() || !dataErrorMessage.IsNullOrEmpty())
            {
                string errorMessage = responseErrorMessage;

                if (!dataErrorMessage.IsNullOrEmpty())
                {
            ...
            if (balanceResponse.Data == null)
            {
                LogUtilService.ForcedDebug($"GetUserScoreReturnModel Data is null, apiResult={apiResult}");

                return new BaseReturnDataModel<UserScore>("GetUserScoreReturnModel Data is null");
            }
        ...
        protected override BaseReturnModel CheckOrCreateRemoteAccount(CreateRemoteAccountParam createRemoteAccountParam)
        {
            ...
            if (balanceResponse.IsSuccess && balanceResponse.Data.Where(w => w.IsSuccess && w.TPGameAccount == createRemoteAccountParam.TPGameAccount).Any())
            {
                return new BaseReturnModel(ReturnCode.Success);
            }
        ...
        protected override BaseReturnDataModel<string> GetRemoteForwardGameUrl(TPGameRemoteLoginParam tpGameRemoteLoginParam)
        {
            ...
            string isAppText = string.Empty;

            if (tpGameRemoteLoginParam.IsMobile)
            {
                isAppText = "&isAPP=true";
            }

            string fullLoginGameUrl = Url.Combine(launchGameModel.Data, $"OType={_sportAppSetting.OddsType}&lang=cs", isAppText);
        ...
        protected override BaseReturnDataModel<string> GetRemoteCreateAccountApiResult(CreateRemoteAccountParam createRemoteAccountParam)
        {
            ...
            {
                Vendor_Member_ID = createRemoteAccountParam.TPGameAccount,
                OperatorId = createRemoteAccountParam.TPGameAccount,
        ...
        protected override BaseReturnDataModel<string> GetRemoteLoginApiResult(TPGameRemoteLoginParam tpGameRemoteLoginParam)
        {
            ...
            string param = $@"vendor_member_id={tpGameRemoteLoginParam.CreateRemoteAccountParam.TPGameAccount}&platform={platform}";
