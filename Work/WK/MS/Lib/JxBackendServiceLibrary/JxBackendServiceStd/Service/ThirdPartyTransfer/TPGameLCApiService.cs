...
namespace JxBackendService.Service.ThirdPartyTransfer
{
    public abstract class TPGameLCApiService : BaseTPGameApiService
    {
        ...
        protected override BaseReturnDataModel<string> GetRemoteLoginApiResult(TPGameRemoteLoginParam tpGameRemoteLoginParam)
        {
            ...
            if (tpGameRemoteLoginParam.LoginInfo != null && !tpGameRemoteLoginParam.LoginInfo.RemoteCode.IsNullOrEmpty())
            {
        ...
        protected override bool IsDoTransferCompensation(string apiResult)
        {
            ...
            if (!tranferResponse.Data.IsSuccess &&
                ...
                tranferResponse.Data.Code.Value == LCResponseCodeType.LC_33.Value)
            {
                return true;
            }
        ...
        private BaseReturnDataModel<string> DoRequest(string param, out DetailRequestAndResponse detail)
        {
            ...
            var webRequestParam = new WebRequestParam()
            {
                ...
                Method = Common.Util.HttpMethod.Get,
            ...
            {
                httpClient.Timeout = TimeSpan.FromMilliseconds((double)webRequestParam.TimeOut);
                HttpResponseMessage response = httpClient.GetAsync(webRequestParam.Url).Result;
                httpStatusCode = response.StatusCode;

                if (response.Content.Headers.ContentType != null)
                {
                    string formatCharSet = response.Content.Headers.ContentType.CharSet.Replace("utf8", Encoding.UTF8.WebName);
                    response.Content.Headers.ContentType.CharSet = formatCharSet;
                }
                ...
                {
                    LogUtilService.Error($"Url = {webRequestParam.Url}, Request Param = {webRequestParam.ToJsonString(ignoreNull: true)}");
