...
namespace JxBackendService
{
    ...
    {
        private readonly Lazy<IThirdPartyUserAccountRep> _thirdPartyUserAccountRep;

        private readonly Lazy<IThirdPartyUserAccountRep> _thirdPartyUserAccountReadRep;
        ...
        protected IUserInfoRep UserInfoRep => _userInfoRep.Value;
        ...
        public BaseTPGameAccountService(EnvironmentUser envLoginUser, DbConnectionTypes dbConnectionType) : base(envLoginUser, dbConnectionType)
        {
            ...
            _thirdPartyUserAccountReadRep = ResolveJxBackendService<IThirdPartyUserAccountRep>(DbConnectionTypes.Slave);
        ...
        public List<UserProductScore> GetAllTPGameUserScores(SearchAllGameUserScoresParam param)
        {
            ...
            List<UserProductScore> returnResult = _jxCacheService.Value.GetCache(searchCacheParam, () =>
            {
                ...
                Dictionary<PlatformProduct, BaseTPGameUserInfo> productUserMap = _thirdPartyUserAccountRep.Value.GetAllTPGameUserInfoMap(param.UserID);
                ...
                foreach (KeyValuePair<string, ITPGameUserInfoService> keyValuePair in s_tpGameUserInfoServiceMap)
                {
                    ...
                    userProductScores.Add(new UserProductScore()
                    {
                        ...
                        ProductName = _platformProductService.Value.GetName(productCode),
                ...
                if (param.IsIncludeMainScores)
                {
                    userProductScores.Add(new UserProductScore()
                    {
                        ...
                        ProductName = _platformProductService.Value.GetName(PlatformProduct.Lottery.Value),
        ...
        public BaseReturnDataModel<long> Create(int userId, PlatformProduct platformProduct, string tpGameAccount, string tpGamePassword)
        {
            ...
            var searchParam = new ThirdPartyUserAccount() { UserID = userId, ThirdPartyType = platformProduct.Value };
            ThirdPartyUserAccount thirdPartyUserAccount = _thirdPartyUserAccountReadRep.Value.GetSingleByKey(InlodbType.Inlodb, searchParam);
            ...
            BaseReturnDataModel<long> returnModel = _jxCacheService.Value.DoWorkWithRemoteLock(CacheKey.ThirdPartyUserAccountLock(userId), () =>
            {
                //這邊要重新再查詢一次，避免第二個請求進來的時候寫入造成pk重複
                thirdPartyUserAccount = _thirdPartyUserAccountRep.Value.GetSingleByKey(InlodbType.Inlodb, searchParam);
                ...
                BaseReturnDataModel<long> createReturnModel = _thirdPartyUserAccountRep.Value.CreateByProcedure(new ThirdPartyUserAccount()
                {
                    UserID = userId,
                ...
                return createReturnModel;
            });

            if (returnModel.IsSuccess)
            {
                _jxCacheService.Value.RemoveCache(CacheKey.ThirdPartyUserAccount(userId));
            }
        ...
        {
            //避免外部用cache判斷造成寫入的時候資料重複,這邊在實際重取一次
            ThirdPartyUserAccount thirdPartyUserAccount = _thirdPartyUserAccountRep.Value.GetSingleByKey(
                InlodbType.Inlodb,
            ...
            bool isSuccess = _thirdPartyUserAccountRep.Value.UpdateByProcedure(thirdPartyUserAccount);
        ...
        public BaseReturnDataModel<UserAccountSearchResult> GetByLocalAccount(int userId)
        {
            ...
            Dictionary<PlatformProduct, BaseTPGameUserInfo> productUserMap = _thirdPartyUserAccountRep.Value.GetAllTPGameUserInfoMap(userId);
            ...
            thirdPartyUserAccounts.ForEach(f =>
            {
                ...
                if (platformProduct != null)
                {
                    ...
                    userAccountSearchReault.TPGameAccountSearchResults.Add(new TPGameAccountSearchResult()
                    {
                        ...
                        TPGameProductName = _platformProductService.Value.GetName(platformProduct.Value),
        ...
        {
            List<ThirdPartyUserAccount> thirdPartyUserAccounts = _thirdPartyUserAccountRep.Value.GetListByTPGameAccount(product, tpGameAccounts);
        ...
        {
            List<ThirdPartyUserAccount> thirdPartyUserAccounts = _thirdPartyUserAccountRep.Value.GetListByTPGameAccount(product: null, new HashSet<string>() { tpGameAccount });

            Dictionary<PlatformProduct, BaseBasicUserInfo> resultMap = thirdPartyUserAccounts.ToDictionary(
                d => _platformProductService.Value.GetSingle(d.ThirdPartyType),
                d => new BaseBasicUserInfo() { UserId = d.UserID });

            List<int> userIdsByRule = resultMap.Select(s => s.Value.UserId).ToList();
            var userIdMap = new Dictionary<int, string>();

            foreach (PlatformProduct platformProduct in _platformProductService.Value.GetNonSelfProduct())
            {
        ...
        private ThirdPartyUserAccount GetSingleThirdPartyUserAccount(int userId, PlatformProduct searchProduct)
        {
            ...
            return _thirdPartyUserAccountRep.Value.GetSingleByUserId(userId, searchProduct);
        }
        ...
        private List<ThirdPartyUserAccount> GetUserThirdPartyUserAccounts(int userId)
        {
            List<ThirdPartyUserAccount> thirdPartyUserAccounts = _jxCacheService
                .Value
                .GetCache(CacheKey.ThirdPartyUserAccount(userId), () => _thirdPartyUserAccountRep.Value.GetListByUserId(userId));
        ...
        public BaseReturnDataModel<UserAccountSearchResult> GetByTPGameAccount(PlatformProduct searchProduct, string tpGameAccount)
        {
            ...
            {
                var platformProductSettingService = ResolveKeyed<IPlatformProductSettingService>(product).Value;
            ...
            List<ThirdPartyUserAccount> thirdPartyUserAccounts = _thirdPartyUserAccountRep.Value.GetListByTPGameAccount(searchTPGameAccounts);
            //資料表內無資料的,查詢各個第三方表
            //把缺資料的產品再找各個第三方的資料, 用規則推算
            List<string> existProductCodes = thirdPartyUserAccounts.Select(s => s.ThirdPartyType).ToList();
            ...
            foreach (PlatformProduct diffPlatformProduct in diffPlatformProducts)
            {
                ...
                {
                    productUserMap = _thirdPartyUserAccountRep.Value.GetAllTPGameUserInfoMap(basicUserInfo.UserId);
                ...
                diffTPGameAccountSearchResults.Add(new TPGameAccountSearchResult()
                {
                    ...
                    TPGameAvailableScore = tpGameUserInfo.AvailableScores,
                    TPGameFreezeScore = tpGameUserInfo.FreezeScores,
                    TPGameProductName = _platformProductService.Value.GetName(diffPlatformProduct.Value),
            ...
            var tpGameAccountSearchResults = thirdPartyUserAccounts.Select(s =>
                {
                    ...
                    return new TPGameAccountSearchResult()
                    {
                        ...
                        TPGameProductName = _platformProductService.Value.GetName(product.Value),
                        Sort = product.Sort
                    };
                }).ToList();
        ...
        private void InitTPGameUserInfoServiceMap()
        {
            lock (s_locker)
            {
                ...
                foreach (PlatformProduct product in GetNonSelfProduct())
                {
                    ...
                    s_tpGameUserInfoServiceMap.Add(product.Value, ResolveJxBackendService<ITPGameUserInfoService>(product).Value);
                }
            }
        }
        ...
        {
            return _platformProductService.Value.GetNonSelfProduct();
        }
    ...
    public class TPGameAccountMSLService : BaseTPGameAccountService
    {
        ...
        public override string GetTPGameAccountByRule(PlatformProduct platformProduct, int userId)
        {
            ...
            else if (platformProduct == PlatformProduct.ABEB)
            {
                string prefixAndEnvCodeUserId = GetPrefixAndEnvCode(EnvCode.AccountPrefixCode, userId);
                tpGameAccount = $"{prefixAndEnvCodeUserId}{ABEBSharedAppSetting.Suffix}";
            }
            else
            {
                //Default encoded string
        ...
        protected override BaseBasicUserInfo GetLocalAccountByRule(PlatformProduct platformProduct, string tpGameAccount)
        {
            ...
            var platformProductSettingService = ResolveKeyed<IPlatformProductSettingService>(platformProduct).Value;
