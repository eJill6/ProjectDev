...
namespace SLPolyGame.Web.Controllers
{
    ...
    {
        private readonly PalyInfo _playInfoService = null;

        private readonly CurrentLotteryInfo _currentLotteryService = null;
        ...
        public AccountController(PalyInfo playInfoService,
        ...
        {
            var detail = this._playInfoService.GetPlayBetByAnonymous(serialNumber);
            ...
            if (detail != null)
            {
                ...
                var currentLotteryInfo = _currentLotteryService.QueryCurrentLotteryInfo(new Model.CurrentLotteryQueryInfo()
                {
                    Count = 1,
        ...
        public ResultModel<MSThirdPartyBettingResult[]> GetTPGameOrders(string startTime, string endTime, string gameId)
        {
            ...
            if (queryOrderGameId == null || !_platformProductService.GetAll().Any(a => a == queryOrderGameId.Product))
            {
                return new ResultModel<MSThirdPartyBettingResult[]>()
                {
                    Success = false,
        ...
        [HttpGet]
        public MiseLiveResponse<List<IGameBalance>> GetTPGameBalances(int userId)
        {
            BaseReturnDataModel<UserAccountSearchReault> returnDataModel = _tpGameAccountReadService.GetByLocalAccount(userId);

            if (!returnDataModel.IsSuccess)
            {
                return returnDataModel.ToMiseLiveResponse<List<IGameBalance>>();
            }

            var allGameBalances = new List<IGameBalance>();
            ...
            {
                PlatformProduct product = _platformProductService.GetSingle(tpGameAccountSearchResult.TPGameProductCode);

                if (map.TryGetValue(product, out HashSet<MiseOrderGameId> miseOrderGameIds))
                {
                    List<IGameBalance> gameBalances = miseOrderGameIds
                        .Select(s =>
                        {
            ...
            return new MiseLiveResponse<List<IGameBalance>>()
            {
                Success = true,
        ...
        {
            var details = this._playInfoService.GetPlayBetsByAnonymous(startTime, endTime, gameId);
            var result = new ResultModel<MSLotteryBettingResult[]>();
            ...
            if (details.Length > 0)
            {
                ...
                if (string.IsNullOrWhiteSpace(gameId))
                {
                    dicCurrentLotteryInfos = details.GroupBy(x => x.LotteryID.Value).ToDictionary(x => x.Key, y =>
                    {
                        ...
                        return _currentLotteryService.QueryCurrentLotteryInfo(query);
                    });
                }
                else
                {
                    ...
                    var currentLotteryInfos = _currentLotteryService.QueryCurrentLotteryInfo(query);
        ...
        private ResultModel<MSThirdPartyBettingResult[]> GetAllGamePlayInfoHistory(DateTime startTime, DateTime endTime, MiseOrderGameId miseOrderGameId)
        {
            ...
            {
                platformProducts.AddRange(_platformProductService.GetAll());
            }
            else
            {
                platformProducts.Add(_platformProductService.GetSingle(param.ProductCode));
            }
            ...
            IAllGameStatService allGameStatService = DependencyUtil.ResolveJxBackendService<IAllGameStatService>(EnvLoginUser, DbConnectionTypes.Slave);
            ...
            ResultModel<MSThirdPartyBettingResult[]> bettingResult = new ResultModel<MSThirdPartyBettingResult[]>
            {
                ...
                Data = pagedResult.ResultList.Select(s =>
                {
                    var msThirdPartyBettingResult = new MSThirdPartyBettingResult
                    {
                        ...
                        Status = BetResultType.GetSingle(s.BetResultType).MiseLiveOrderStatus.ToString(),
                        ProfitLoss = s.WinMoneyText,
                        GameName = miseOrderGameId.Name,
                        GameDetail = s.Memo,
                        PeriodNumber = string.Empty,
                        GameResult = s.Memo,
                        Turnover = s.BetMoneyText,
                    };
