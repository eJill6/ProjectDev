<template>
  <div class="h-100 second-content rounded-main">
    <div class="h-100">
      ...
      <div class="lottery_follow border-bottom">
...
export default defineComponent({
  ...
  computed: {
    ...
    followOrderType() {
      ...
      return info[this.$store.state.lotteryInfo.gameTypeName] || [];
    },
    className(): { [key: string]: string[] } {
      return {
        "bg-cyan text-white": ["单", "大"],
        "bg-orange text-white": ["双", "小"],
        "bg-white text-black": ["龙", "虎", "和"],
        "bg-red text-white": ["红"],
        "bg-green text-white": ["绿"],
        "bg-blue text-white": ["蓝"]
      };
    },
    gameTypeClass() {
      const gameType: {
