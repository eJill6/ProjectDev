...
export default defineComponent({
    ...
    methods: {
        calculateVirtualScroll() {
            ...
            let end = Math.ceil(((itemElemHeight * itemCountPerView) + container.scrollTop) / itemElemHeight);

            this.virtualScroll.list = this.list.slice(start, end);

            this.virtualScroll.paddingTop = start * itemElemHeight;
            this.virtualScroll.paddingBottom = ((this.list.length - this.virtualScroll.list.length) * itemElemHeight) - this.virtualScroll.paddingTop;
        },
        onScroll(e: Event) {
            let { scrollTop, offsetHeight, scrollHeight } = e.target as HTMLDivElement;
