...
type GetterType = {
  ...
  formattedIssueNoCountdownTime(state: State): string;
  formattedClosureCountdownTime(state: State): string;
...
export const getters: GetterTree<State, State> & GetterType = {
  ...
  formattedIssueNoCountdownTime(state) {
    let source = state.issueNoSecondLeft;
  ...
  formattedClosureCountdownTime(state) {
    let source = state.closureSecondLeft;
    let minutes = Math.floor(source / 60);
    let seconds = source % 60;
