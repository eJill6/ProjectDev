    ...
    <div class="overflow no_scrollbar post bg_post_c">
      ...
      <div class="padding_basic">
        <div class="introduce_title">
          <img src="@/assets/images/post/pic_title_c_1.png" alt="" />
        </div>
        <div class="introduce_text" v-html="messages.what"></div>
        <div class="introduce_title">
          <img src="@/assets/images/post/pic_title_c_2.png" alt="" />
        </div>
...
export default defineComponent({
  ...
  methods: {
    verifyIdentity() {
      if (this.canPost(PostType.Official)) {
        this.navigateToFrom(PostType.Official);
      } else {
        const info: TipInfo = {
    ...
    async getWhatIs() {
      try {
        this.messages = await api.getWhatIs(PostType.Square, AdvertisingContentType.Official);
      } catch (error) {
        toast(error);
      }
    },
