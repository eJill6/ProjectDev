...
export interface NuiNuiPoker extends PokerResult {
  /**
...
export class NuiNui extends PokerRule {
  ...
  public confirmResult(pokers: string[]) {
    ...
    const blueArea = this.sortCardType(pokers.slice(0, this.gameAreaTotal()));
    const redArea = this.sortCardType(pokers.slice(this.gameAreaTotal()));
   ...
   */
  public sortCardType(pokers: string[]): NuiNuiPoker {
    if (pokers.length !== this.gameAreaTotal()) {
      throw new Error(`单方牌数错误`);
    }

    let result: NuiNuiPoker = {
      cards: [],
      isWin: false,
      imageType: "0",
      victoryConditions: {
    ...
    if (result.victoryConditions.weight === NuiNuiWeight.nuiNui) {
      imageType = "nui";
    } else if (result.victoryConditions.weight === NuiNuiWeight.noNui) {
      imageType = 'none'
