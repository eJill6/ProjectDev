...
export default defineComponent({
  ...
  async created() {
    ...
    const pageRedirectInfo = api.getPageRedirectInfo();
    const isRedirect = pageParamInfo.startsWith("Redirect_");
    if (
      pageRedirectInfo &&
      redirectParamInfo.has(pageRedirectInfo.redrectType)
    ) {
      this.redirectPage(pageRedirectInfo);
    } else if (!isRedirect) {
  ...
  methods: {
    ...
    redirectPage(pageRedirectInfo: PageRedirectInfo) {
      if (redirectParamInfo.has(pageRedirectInfo.redrectType)) {
        let param = redirectParamInfo.get(pageRedirectInfo.redrectType);
        if (param?.queryName == "applyId") {
          this.$router.push({
            name: param?.redrectPage,
            query: {
              applyId: pageRedirectInfo.refId,
            },
          });
        } else if (param?.queryName == "postId") {
          this.$store.commit(MutationType.SetBanner, []);
          this.$router.push({
            name: param?.redrectPage,
            query: {
              postId: pageRedirectInfo.refId,
            },
          });
        } else {
          this.$router.push({ name: param?.redrectPage });
        }
      }
    },
    judgeDevice() {
