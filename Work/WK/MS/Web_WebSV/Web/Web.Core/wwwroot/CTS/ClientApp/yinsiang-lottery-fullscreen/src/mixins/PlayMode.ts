...
export default defineComponent({
  methods: {
    async changePlayModeAsync(playConfig: PlayConfig) {
      if (this.$store.state.currentPlayModeId === playConfig.playModeId) return;

      let defaultPlayTypeInfos = playConfig.playTypeInfos.filter(
    ...
    async changePlayTypeAsync(playTypeInfo: PlayTypeInfo) {
      ...
      let typeModelPlayTypeRadios =
        playTypeInfo.playTypeRadioInfos[defaultTypeModel];
    ...
    async changePlayTypeRadioAsync(playTypeRadioInfo: PlayTypeRadioInfo) {
      if (
        this.$store.state.currentPlayTypeRadioId ===
        playTypeRadioInfo.info.playTypeRadioID
      )
        return;
      ...
      let rebatePros = await api.getRebateProAsync(
        lotteryId,
        playTypeId,
        playTypeRadioId
      );
    ...
    createChangeLongGameDateTime(models: IssueNo[]) {
      let allChangLongInfo: ChangLongDateTimeModel[] = [];
      models.forEach((item) => {
        let endTimeValue = new Date(item.endTime).getTime();
        let startTimeValue = new Date(item.currentTime).getTime();
        let secondsLeft = Math.abs((endTimeValue - startTimeValue) / 1000);
        const dateTimeModel: ChangLongDateTimeModel = {
          lotteryId: item.lotteryId,
          timeRule: TimeRules.issueNoCountdown,
          secondsTotal: secondsLeft,
          secondsTenDigits: `${Math.floor(secondsLeft / 10)}`,
          secondsDigits: `${secondsLeft % 10}`,
        };
        allChangLongInfo.push(dateTimeModel);
      });
      this.$store.commit(
