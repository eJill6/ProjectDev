<template>
  <div class="pl-7 pr-7">
    ...
    >
      ...
      >
        <button
          class="d-flex align-items-center justify-content-center bet-btn bet-btn-s bet-btns1_S"
          :class="getClass(fieldIndex, numberIndex)"
          @click="toggleSelectNumber(fieldIndex, numberIndex)"
        >
          <div
            class="nuinui-option_S"
            :class="number.length > 2 ? `fs-2` : `fs-6`"
          >
            {{ number }}
          </div>
...
export default defineComponent({
  ...
  methods: {
    getClass(fieldIndex: number, numberIndex: number) {
      ...
      return isActive ? "bet-btns1_S_active" : "bet-btns1_S";
    },
  },
});
</script>
