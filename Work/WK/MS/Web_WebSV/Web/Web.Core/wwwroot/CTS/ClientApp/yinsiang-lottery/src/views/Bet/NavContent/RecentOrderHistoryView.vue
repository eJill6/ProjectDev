<template>
  <div class="h-history-list overflow-scroll-y no-scrollbar position-relative">
    <table class="w-100 fs-3 bet-h-table" v-if="!isLoading && list.length">
      <tbody>
        <tr v-for="item in list">
          ...
          <td>
            <span class="text-bet-lose-darkbg">{{ item.noteMoneyText }}</span>
          </td>
          <td>
            <span class="text-bet-win-darkbg">{{
              getPrizeMoneyText(item)
            }}</span>
          </td>
          <td>
            <div
              class="d-flex justify-content-center align-items-center cusror-pointer p-3-5"
              @click="betAsync(item)"
            >
    ...
    >
      <div class="position-relative ic_default size-s"></div>
