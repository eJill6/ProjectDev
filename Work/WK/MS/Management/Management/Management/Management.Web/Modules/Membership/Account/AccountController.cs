...
using JxBackendService.Model.Entity.BackSide;
...
namespace Management.Membership.Pages
{
    ...
    public partial class AccountController : Controller
    {
            ...
            [FromServices] IApiService apiService)
        {
            return this.ExecuteMethod(() =>
            {
                ...
                BaseReturnDataModel<ValidateLoginTokenResult> apiResult = apiService.ValidateLoginToken(new ValidateLoginTokenRequest { Token = request.Token });

                if (!apiResult.IsSuccess)
                {
                    throw new ValidationError(apiResult.Code, apiResult.Message);
                }

                string username = apiResult.DataModel.UserName;
                var result = passwordValidator.Validate(ref username, apiResult.DataModel.Password);
                ...
                if (result == PasswordValidationResult.Valid)
                {
                    ...
                    Cache.SetLocalStoreOnly($"LoginUser:{principal.GetIdentifier()}", TimeSpan.FromDays(1), UserRow.Fields.GenerationKey, request.Token);
