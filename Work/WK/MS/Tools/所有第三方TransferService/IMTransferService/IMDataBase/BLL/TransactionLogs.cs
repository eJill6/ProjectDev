...
namespace IMDataBase.BLL
{
    public class TransactionLogs : BaseTransactionLogs<IMApiParamModel, BetLogResult<List<BetResult>>>
    {
        ...
        {
            return DependencyUtil.ResolveKeyed<IIMBetDetailService>(platformMerchant);
        }

        public void GetBetInfo(IMApiParamModel model)
        {
            string lastSearchToken = IMProfitLossInfo.SelectLastSearchTime();
            ...
            foreach (IMProductCode productCode in IMProductCode.GetAll())
            {
                ...
                else if (result.Code == (int)APIErrorCode.NoDataFound)
                {
                    ...
                    LogsManager.Info("目前查询" + productCode.ToString()
                ...
                else
                {
                    ...
                    LogsManager.Info("获取" + productCode.ToString()
        ...
        private bool CycleTryOrder(List<BetResult> betDetails)
        {
            try
            {
                ...
                foreach (BetResult betinfo in betDetails)
                {
                    ...
                    if (!IMProfitLossInfo.ExistsOrder(bet.BetId))
                    {
                        string memo = GetMemo(betinfo);
                        IMProfitLossInfo.SaveDataToLocal(bet, memo);
                    }
                }
        ...
        /// </summary>
        /// <param name="ts"></param>
        private static bool ResetingNextSearchTime(string lastGameEndTs)
        {
            try
            {
