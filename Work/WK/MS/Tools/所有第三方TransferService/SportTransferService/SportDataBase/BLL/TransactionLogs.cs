...
namespace SportDataBase.BLL
{
    public class TransactionLogs : BaseTransactionLogs<SportApiParamModel, ApiResult<BetResult>>
    {
        ...
        {
            return DependencyUtil.ResolveKeyed<ISportBetDetailService>(platformMerchant);
        }
        ...
        {
            string last_version_key = SportProfitLossInfo.SelectVersion_key();
        ...
        private void GetBetDetails(string last_version_key)
        {
            ...
            {
                LogsManager.ForcedDebug($"获取下注信息失败,返回状态码：{betDetail.error_code},返回信息：{betDetail.message}");
            }
            else
            {
                LogsManager.ForcedDebug("获取下注信息失败 betDetail is null");
            }
            ...
            {
                SportProfitLossInfo.UpdateVersion_key(updateVersionKey);
            }
        }
        ...
        private bool CycleTryOrder(BetResult betResult)
        {
            ...
            foreach (ISabaSportBetDetailInfo betInfo in allBetInfos)
            {
                //不是我們的不處理
                if (!allUserMap.ContainsKey(betInfo.Vendor_member_id))
                {
                    continue;
                }

                if (!IsTicketStatusValid(betInfo))
                {
                    continue;
                }

                if (!SportProfitLossInfo.ExistsOrder(betInfo.Trans_id))
                {
                    if (!SaveDataToLocal(betInfo))
                    {
                        return false;
                    }
                }
        ...
        private bool SaveDataToLocal(ISabaSportBetDetailInfo detailInfo)
        {
            #region 参数
            ...
            {
                detailInfo.Match_datetime = match_datetime1.AddHours(12).ToString();
            }
            ...
            {
                detailInfo.Transaction_time = transaction_time1.AddHours(12).ToString();
            }
            ...
            {
                detailInfo.Winlost_datetime = winlost_datetime1.AddHours(12).ToString();
            }
            ...
            {
                detailInfo.Settlement_time = settlement_time1.AddHours(12).ToString();
            }
            ...
            ISabaSportBetDetailName detailName = new BetDetailName()
            {
                ...
                HomeName = homeName,
                AwayName = awayName,
                BetTypeText = betTypeName,
                LeagueName = leagueName,
                Memo = localizationParam.ToLocalizationJsonString()
            };

            return SportProfitLossInfo.SaveDataToLocal(detailInfo, detailName);
        }
