...
namespace UnitTestProject
{
    ...
    public class PlatformServiceTest : BaseUnitTest
    {
        ...
        [TestMethod]
        public void TestLaunchGetLaunchURLHTML()
        {
            var httpWebRequestUtilService = DependencyUtil.ResolveService<IHttpWebRequestUtilService>().Value;

            string response = httpWebRequestUtilService.GetResponse(
                new WebRequestParam()
                {
                    Purpose = "TestLaunchGetLaunchURLHTML",
                    Url = "https://api.pg-bo.me/external-game-launcher/api/v1/GetLaunchURLHTML",
                    ContentType = HttpWebRequestContentType.WwwFormUrlencoded,
                    Body = "operator_token=eb27086b1900c8470f7ee302af16fc06&path=%2fweb-lobby%2f&extra_args=ops%3D33EB5DEF172E50170EA2FCE696DA210FDF8B58E2FC1F72A0F26B3F51B479F6E77B70AA82D78072D96FA382AF808F689B2AE3F9114AB7AAE899AEB68807FD8AA6C7F1C7659E24BE7072587D3A58F4AA9DE9A81BA8ED8413D0%26l%3Dzh&url_type=web-lobby&client_ip=18.166.145.49",
                    Method = JxBackendService.Common.Util.HttpMethod.Post
                },
                out HttpStatusCode httpStatusCode);

            using (StreamWriter streamWriter = new StreamWriter("pgsl.html"))
            {
        ...
        {
            BaseReturnDataModel<RequestAndResponse> returnModel = _tpGameApiService.Value.GetRemoteBetLog("1661937032000");
        }
        ...
        [TestMethod]
        public void TestEnqueueTransferToMiseLiveMessage()
        {
            var internalMessageQueueService = DependencyUtil.ResolveService<IInternalMessageQueueService>().Value;

            internalMessageQueueService.EnqueueTransferToMiseLiveMessage(new TransferToMiseLiveParam()
            {
                UserID = EnvironmentUser.LoginUser.UserId,
                Amount = 10,
            });
