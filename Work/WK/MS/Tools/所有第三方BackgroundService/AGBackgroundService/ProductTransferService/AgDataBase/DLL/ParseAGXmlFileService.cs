...
namespace ProductTransferService.AgDataBase.DLL
{
    ...
    public class ParseAGXmlFileService : IParseAGXmlFileService
    {
                ...
                try
                {
                    ...
                    if (dataType == "TR")
                    {
                        ...
                        if (flag == "0" && (transferType == "IN" || transferType == "OUT"))
                        {
                            ...
                            agInfo.betTime = ConvertAGDateTimeToChinaDateTime(betTime);
                            ...
                            agInfo.recalcuTime = ConvertAGDateTimeToChinaDateTime(recalcuTime);
                    ...
                    else if (dataType == "HSR")
                    {
                        ...
                        if (flag == "0" && (type == "1" || type == "2" || type == "7"))
                        {
                            ...
                            agFishInfo.SceneStartTime = ConvertAGDateTimeToChinaDateTime(SceneStartTime);
                            agFishInfo.SceneEndTime = ConvertAGDateTimeToChinaDateTime(SceneEndTime);
                            ...
                            agFishInfo.creationTime = ConvertAGDateTimeToChinaDateTime(creationTime);
        ...
        private DateTime ConvertAGDateTimeToChinaDateTime(string dateTimeString)
        {
            if (!DateTimeOffset.TryParse(dateTimeString, out DateTimeOffset dateTimeOffset))
            {
                throw new ArgumentException($"dateTimeString format is can't parse. {dateTimeString}");
            }

            var agDateTimeOffset = new DateTimeOffset(
                dateTimeOffset.Year, 
                ...
                new TimeSpan(-4, 0, 0)); //API server 的时区是GMT-4

            return agDateTimeOffset.UtcDateTime.ToChinaDateTime();
        }
    }
}