...
namespace MMBatch.Infrastructure.Quartz
{
    ...
    /// 將圖片加密的排程
    /// </summary>
    public class ImageDecryptJob : BaseJob
    {
        /// <summary>
        /// 開始時間
        /// </summary>
        ...
        /// 日誌
        /// </summary>
        private readonly ILogger<ImageDecryptJob> _logger = null;
        ...
        public ImageDecryptJob(IEnumerable<IMediaService> services,
            ILogger<ImageDecryptJob> logger)
        {
            _services = services;
            _logger = logger;
        }
        ...
        public override async Task Execute(IJobExecutionContext context)
        {
            ...
            if (!_isDecrypted)
            {
                _logger.LogInformation($"Begin Decrypt, From:{_begin}, To:{_end}");
                var services = _services.Where(x => x.Type == MediaType.Image);
                foreach (var service in services)
                {
                    await service.Decrypt(_begin, _end, _finish, false);
                ...
                {
                    _isDecrypted = true;
                    _logger.LogInformation($"Encrypt Job Finish, _end:{_end}, now:{_finish}");
                }
                else
                {
                    _end = _begin;
