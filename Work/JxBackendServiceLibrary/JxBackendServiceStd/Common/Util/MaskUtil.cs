...
namespace JxBackendService.Common.Util
{
    ...
    {
        public static readonly string MaskToken = "*";
        ...
        {
            return ToMaskMiddleWithHeadEnd(input, 4);
        }
        ...
        /// <summary> 貨幣錢包遮罩，顯示前後五碼，剩下遮罩 </summary>
        public static string ToMaskCoinWalletAddress(this string input)
        {
            return ToMaskMiddleWithHeadEnd(input, 5);
        }

        private static string ToMaskMiddleWithHeadEnd(this string input, int displayHeadEndCount)
        {
            ...
            int totalDisplayCount = displayHeadEndCount * 2;

            HashSet<int> visibleIndexList = new HashSet<int>();
            int totalCount = input.Length;

            if (totalCount > totalDisplayCount)
            {
                var vs = new List<int>();

                for (int idx = 0; idx < displayHeadEndCount; idx++)
                {
                    vs.Add(idx);
                }

                for (int idx = (totalCount - displayHeadEndCount); idx < totalCount; idx++)
                {
                    vs.Add(idx);
                }

                vs.ForEach(f => visibleIndexList.Add(f));
            }
            else
            {
                int visiblePartLength = visiblePartLength = (int)(Math.Floor(Convert.ToDouble(input.Length) / 3));
