...
namespace JxBackendService.Service.TransferRecord
{
    ...
    {
        private readonly IAllTPTransferRecordRep _allTPTransferRecordRep;
        ...
        {
            List<PlatformProduct> nonSelfProducts = _platformProductService.GetNonSelfProduct();
        ...
        public PagedResultModel<TransferRecordViewModel> GetTransferRecord(SearchTransferRecordParam param)
        {
            ...
            {
                queryTPParam.PlatformProducts = _platformProductService.GetNonSelfProduct();
            }
            else
            {
                PlatformProduct product = _platformProductService.GetSingle(param.ProductCode);
                queryTPParam.PlatformProducts = new List<PlatformProduct> { product };
                queryPlatformParam.PlatformProduct = product;
            }

            PagedResultModel<TransferRecordViewModel> tpTransferRecord = _allTPTransferRecordRep.GetAllTPTransferRecord(
            ...
            foreach (SearchTransferType transferType in searchTransferTypes)
            {
                if (transferType == SearchTransferType.In)
                {
                    ...
                    var cmoneyInInfoRep = ResolveJxBackendService<ICMoneyInInfoRep>();
                ...
                else if (transferType == SearchTransferType.Out)
                {
                    ...
                    var cmoneyOutInfoRep = ResolveJxBackendService<ICMoneyOutInfoRep>();
