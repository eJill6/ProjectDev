    ...
    public class JxCacheService : IJxCacheService
    {
            ...
            lock (locker)
            {
                switch (GetCacheType(searchCacheParam.Key))
                {
                    ...
                    case CacheTypes.LocalAndCacheServer:
                        ...
                        double localMemoryCacheSeconds = _tempLocalMemoryCacheSeconds;
        ...
        public void SetCache<T>(SetCacheParam setCacheParam, T value) where T : class
        {
            ...
            switch (GetCacheType(setCacheParam.Key))
            {
                ...
                case CacheTypes.LocalAndCacheServer:
                    MemoryCacheUtil.SetCache(fullCacheKey, value, _tempLocalMemoryCacheSeconds, _isTempLocalMemorySliding);
                    _redisService.SetCache(setCacheParam.Key.DbIndex, fullCacheKey, value, setCacheParam.CacheSeconds,
                        setCacheParam.Key.IsDoSerialize);
        ...
        public void DoWorkWithRemoteLock(CacheKey cacheKey, Action work)
        {
            ...
            _redisService.DoWorkWithLock(GetFullCacheKeyValue(cacheKey), work);
        }
        ...
        public T DoWorkWithRemoteLock<T>(CacheKey cacheKey, Func<T> work)
        {
            ...
            return _redisService.DoWorkWithLock(GetFullCacheKeyValue(cacheKey), work);
        }
