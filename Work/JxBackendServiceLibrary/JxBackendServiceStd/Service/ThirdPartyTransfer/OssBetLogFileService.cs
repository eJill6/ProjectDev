...
namespace JxBackendService.Service.ThirdPartyTransfer
{
    public class OssBetLogFileService : BaseService, IBetLogFileService
    {
        ...
        {
            var coreOssSettingService = ResolveJxBackendService<IOSSSettingService>(DbConnectionTypes.Slave);
        ...
        public RequestAndResponse GetBetLogContent(PlatformProduct product, long lastFileSeq)
        {
            return RetryJobUtil.GetJobResultWithRetry(() =>
            {
                ...
                List<string> fullFileNames = _objectStorageService.GetFullFileNames(string.Format(s_remoteProductFolderTemplate, merchantCode, product));
                fullFileNames.Sort();
                ...
                foreach (string fullFileName in fullFileNames)
                {
                    ...
                    else
                    {
                        ...
                        _logUtilService.ForcedDebug($"DeleteOssFile: {fullFileName}");
                ...
                // 讀檔回來處理
                byte[] bytes = _objectStorageService.GetObject(downloadFileName);
        ...
        public void WriteRemoteContentToOtherMerchant(PlatformProduct product, long fileSeq, string content, List<string> copyToMerchantCodes)
        {
            ...
            foreach (string merchantCode in copyToMerchantCodes)
            {
                ...
                _objectStorageService.UploadObject(bytes, uploadFullFileName);
                _logUtilService.ForcedDebug($"UploadObject:{uploadFullFileName}");
            }
        }
        ...
        public void WriteRemoteContentToOtherMerchant(PlatformProduct product, string remoteSubfolderFilePath, string fileToUpload, List<string> copyToMerchantCodes)
        {
            ...
            foreach (string merchantCode in copyToMerchantCodes)
            {
                ...
                _objectStorageService.UploadObjectByFilePath(uploadFullFileName, fileToUpload);
                _logUtilService.ForcedDebug($"UploadObjectByFilePath:{uploadFullFileName}");
            }
        }
        ...
        public List<string> GetFullFileNames(PlatformProduct product, string remoteSubfolderPath)
        {
            ...
            List<string> fullFileNames = _objectStorageService.GetFullFileNames(filter);
        ...
        {
            byte[] bytes = _objectStorageService.GetObject(fullFileName);
            string fileContent = Encoding.UTF8.GetString(bytes);
        ...
        {
            _objectStorageService.DeleteObject(fullFileName);
        }

        public void DeleteFiles(List<string> fullFileNames)
        {
            _objectStorageService.DeleteObjects(fullFileNames);
        }
    }
}