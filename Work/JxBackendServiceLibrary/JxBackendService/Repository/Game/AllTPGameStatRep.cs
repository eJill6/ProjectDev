    ...
    public class AllTPGameStatRep : BaseDbRepository, IAllTPGameStatRep
    {
        ...
        public PagedResultWithAdditionalData<AllGamePlayInfoRowModel, TPGamePlayInfoStatModel> GetPagedAllPlayInfo(SearchAllPagedPlayInfoParam searchParam)
        {
            ...
            foreach (PlatformProduct platformProduct in searchParam.PlatformProducts)
            {
                ...
                SqlSelectColumnInfo entityJsonColumnInfo = null;
                string entityJsonColumnName = nameof(AllGamePlayInfoRowModel.EntityJson);

                if (platformProduct == PlatformProduct.Lottery)
                {
                    entityJsonColumnInfo = new SqlSelectColumnInfo() { ColumnName = ModelUtil.GetColumnValueJsonSql<PalyInfo>(), AliasName = entityJsonColumnName };
                }
                else
                {
                    entityJsonColumnInfo = new SqlSelectColumnInfo() { ColumnName = "NULL", AliasName = entityJsonColumnName };
                }

                aliasColumnMap.Add(entityJsonColumnName, entityJsonColumnInfo);
            ...
            int fetchCount, string statFilters, string rowFilters)
        {
            ...
            string entityJsonColumnName = columnInfoMap[nameof(AllGamePlayInfoRowModel.EntityJson)].ColumnName;
            ...
            sql.AppendLine($@"
                ...
                BEGIN
                    ...
                    SELECT TOP({fetchCount})
                        ...
                        {entityJsonColumnName} AS EntityJson
                    FROM {fullTableName} WITH(NOLOCK)
                    WHERE
                        {rowFilters}
