...
namespace JxBackendService
{
    public abstract class BaseTPGameAccountService : BaseService, ITPGameAccountService, ITPGameAccountReadService
    {
        ...
        public BaseReturnDataModel<long> Create(int userId, PlatformProduct platformProduct, string tpGameAccount)
        {
        ...
        /// 從平台帳號查第三方帳號與餘額
        /// </summary>
        public BaseReturnDataModel<UserAccountSearchReault> GetByLocalAccount(int userId)
        {
            ...
            {
                // 取得主帳戶餘額
                PlatformAccountSearchResult = GetPlatformAccountSearchResult(userId)
            };

            //先找ThirdPartyUserAccount中有的資料
            List<ThirdPartyUserAccount> thirdPartyUserAccounts = GetUserThirdPartyUserAccounts(userId)
                .Where(w => GetNonSelfProduct().Any(p => p.Value == w.ThirdPartyType)).ToList();
            ...
            thirdPartyUserAccounts.ForEach(f =>
            {
                ...
                {
                    UserScore userScore = GetTPGameUserScore(platformProduct.Value, userId);
        ...
        public BaseReturnDataModel<string> GetTPGameAccountByLocalAccount(int userId, PlatformProduct searchProduct)
        {
            ...
            {
                tpgameAccount = GetTPGameAccountByRule(searchProduct, userId);
            }
        ...
        public BaseReturnDataModel<UserAccountSearchReault> GetByTPGameAccount(PlatformProduct searchProduct, string tpGameAccount)
        {
            ...
            foreach (PlatformProduct diffPlatformProduct in diffPlatformProducts)
            {
                ...
                //找第三方userInfo有無資料
                Dictionary<PlatformProduct, BaseTPGameUserInfo> productUserMap = _thirdPartyUserAccountRep.GetAllTPGameUserInfoMap(basicUserInfo.UserId);
                bool isUserExists = productUserMap.ContainsKey(diffPlatformProduct);
                ...
                thirdPartyUserAccounts.Add(new ThirdPartyUserAccount()
                {
                    ...
                    Account = GetTPGameAccountByRule(diffPlatformProduct, basicUserInfo.UserId)
                });
            }
            ...
            if (!userAccountSearchReault.TPGameAccountSearchResults.Any())
            {
                ...
                return new BaseReturnDataModel<UserAccountSearchReault>(ReturnCode.SearchResultIsEmpty, userAccountSearchReault);
            }
